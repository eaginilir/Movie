...
def calculate_revenue(predictions: list, answers: list, revenue_table: dict) -> float:
    ...
    total_revenue = 0
    common_keys = set(pred_dict.keys()) & set(ans_dict.keys())

    if common_keys:
        pairs = [(pred_dict[key], ans_dict[key]) for key in common_keys]
    else:
        # Some released sample files use placeholder ids that do not match.
        # Fall back to positional scoring so the local self-test remains usable.
        pairs = [
            (p['rating'], a['rating'])
            for p, a in zip(predictions, answers)
        ]

    for pred_rating, true_rating in pairs:
...
def calculate_metrics(predictions: list, ground_truth: list) -> dict:
    ...
    if common_keys:
        pairs = [(pred_dict[k], truth_dict[k]) for k in common_keys]
    else:
        pairs = [
            (float(p['rating']), float(t['rating']))
            for p, t in zip(predictions, ground_truth)
        ]
        if pairs:
            print("  [提示] 预测与答案ID无交集，已按样本顺序计算指标")
        else:
            return None

    errors = [abs(pred - truth) for pred, truth in pairs]
    num_samples = len(errors)

    return {
        'num_samples': num_samples,
        'mae': sum(errors) / num_samples,
        'rmse': math.sqrt(sum(e ** 2 for e in errors) / num_samples),
        'accuracy_exact': sum(1 for pred, truth in pairs if pred == truth) / num_samples * 100,
        'accuracy_0.5': sum(1 for e in errors if e <= 0.5) / num_samples * 100,
        'accuracy_1.0': sum(1 for e in errors if e <= 1.0) / num_samples * 100,
    }
...
def call_glm_api(system_prompt: str, user_prompt: str, api_key: str,
    ...
    try:
        ...
        try:
            ...
            input_tokens = usage.get("prompt_tokens", est_input_tokens)
            output_tokens = usage.get("completion_tokens", len(result))

            print(f"  [样本{sample_index}] API响应成功(HTTP), 耗时{elapsed:.1f}s, 输入{input_tokens}tok, 输出{output_tokens}tok")
            return result, input_tokens, output_tokens
