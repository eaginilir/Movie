def generate_prompt(ctx):
    ...
    hardcoding). This is the grid-searched candidate after the 98.01 online
    submission.  It keeps the same low-token interaction pattern, but uses the
    best robust local hyper-parameters found by warm LOO + 5-seed official-like
    validation:

      * ordinary Python rounding instead of the conservative calibrated threshold;
      * public-score blend weight 0.37;
      * tag-neighbor confidence = min(tag_weight / 8, 0.6);
      * no same-director bonus, because it was noisy on sparse 9-item histories;
      * keep the user's demonstrated rating range guardrail.

      * Warm (user has history): a tag-neighbor collaborative
        anchor blended toward the user's own mean, lightly pulled by the public
        score. GLM-4.5-air (think off) is worse than this deterministic anchor
        on warm samples because it regresses/overshoots user-specific habits, so
    ...
    def warm_anchor(history, target, target_info, stats, public_score):
        """Tag-neighbor CF rating blended toward user mean and public score.
        Tuned by local grid search: no director bonus, conf cap=0.6, div=8,
        pubw=0.37."""
        ...
        if wsum > 0:
            ...
            anchor = stats["avg"]
        if public_score > 0:
            anchor = anchor * 0.63 + (public_score / 2.0) * 0.37
    ...
    if stats["count"] == 0:
        ref = clamp_rating(cold_reference(public_score))
    ...
    anchor = clamp_rating(warm_anchor(history, target, target_info, stats, public_score))
